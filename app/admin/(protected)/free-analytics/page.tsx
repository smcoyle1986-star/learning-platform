...
function List({ title, description, items, empty }: { title: string; description: string; items: FreeAnalyticsItem[]; empty: string }) {
  return (
...
export default async function FreeAnalyticsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const cards = [
    ...
    { label: "Games completed", value: analytics.summary.completions, helper: `${analytics.summary.completionRate}% of starts`, icon: Trophy },
    { label: "Signups started", value: analytics.summary.signupStarts, helper: "From Free Games routes", icon: MousePointerClick },
    { label: "Accounts created", value: analytics.summary.signupCompletions, helper: `${analytics.summary.signupRate}% of hub sessions`, icon: UserPlus },
    { label: "Animals Demo starts", value: analytics.summary.demoStarts, helper: "Separate demo lesson path", icon: Presentation },
  ];
  const max = Math.max(1, ...analytics.trend.map((item) => Math.max(item.starts, item.completions, item.signups)));
  ...
  return (
    <section className="px-5 py-8 sm:px-7 lg:px-10 lg:py-10">
      <div className="border-b border-[#dfe4dc] pb-7">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-[#6e786d]">See how consented visitors move from the Free Games hub into a game, then through account signup. Counts are unique browser sessions, not repeated clicks.</p>
      </div>
      ...
      <article className="mt-5 rounded-2xl border border-[#dfe4dc] bg-white p-5">
        <div><h2 className="font-semibold text-[#394439]">Unique-session funnel</h2><p className="mt-1 text-xs leading-5 text-[#7a8379]">The percentage compares each stage with the immediately preceding stage. Direct entries can make a later stage larger than the one before it.</p></div>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-2">
        <List title="Campaigns and referring sites" description="First-touch UTM campaign, then source, then referring site. Each session appears once per label." items={analytics.campaigns} empty="Campaign data will appear after a consented visitor reaches Free Games." />
        <List title="Countries" description="Vercel’s anonymous two-letter country context. Local development will show unknown." items={analytics.countries} empty="Country data will appear after production activity." />
        <List title="Most played Free Games" description="Unique sessions that started each game." items={analytics.games} empty="No Free Games starts recorded yet." />
        <List title="Most used topics" description="Unique sessions that started a public topic." items={analytics.topics} empty="No public topics started yet." />
      ...
      <article className="mt-5 rounded-2xl border border-[#dfe4dc] bg-white p-5">
        <div><h2 className="font-semibold text-[#394439]">Daily games and signup outcomes</h2><p className="mt-1 text-xs text-[#7a8379]">Unique sessions per day for game starts, completed games, and server-confirmed accounts.</p></div>
