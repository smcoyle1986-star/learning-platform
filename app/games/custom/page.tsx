...
export default function CustomGamePage() {
  ...
  return <main className="mx-auto max-w-2xl px-6 py-12 sm:py-20"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#718267]">Your vocabulary · {GAME_NAMES[game]}</p><h1 className="mt-3 text-3xl font-bold">Make this game fit your lesson</h1>
    ...
    <section className="mt-7 rounded-2xl border border-[#dce5d8] bg-white p-6">
      {loading ? <p>Loading your account…</p> : !user ? <><p>Create a free account. Confirm your email to unlock two weeks of Premium, including custom vocabulary in every game.</p><Link className="btn btn-primary mt-5" href={`/signup?next=${encodeURIComponent(returnPath)}`}>Create free account</Link></> : usable ? <><p>{access?.isPremium ? "Your Premium access includes custom vocabulary in every game." : "This week’s featured game includes your own vocabulary for free."}</p><Link onClick={prepare} className="btn btn-primary mt-5" href={buildUrl(game)}>Choose my vocabulary</Link></> : <>
