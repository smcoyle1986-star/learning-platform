...
export default function AnimalsVocabularyFreeResourcePage() {
  return (
    <main className="min-h-screen bg-[#f7f6f2] text-[#2f3a2f]">
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-6 pb-16 pt-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:pt-16">
        <div>
          ...
          <h1 className="mt-4 max-w-3xl text-4xl font-semibold tracking-tight md:text-6xl">Animals Vocabulary - Beginner ESL</h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-[#596456]">A complete six-word animal vocabulary lesson for beginner English learners. Print it, teach it, and use it with no account required.</p>
          <div className="mt-8 flex flex-wrap gap-3">
            <a href="/free-resources/animals-vocabulary-beginner-esl.pdf" download className="btn btn-primary px-6 py-3 text-base"><Download size={19} />Download the free PDF</a>
      ...
      <section className="border-y border-[#e2e8dd] bg-white">
        <div className="mx-auto grid max-w-7xl gap-5 px-6 py-10 sm:grid-cols-2 lg:grid-cols-4">
          {["10-page printable PDF", "A0-A1 beginner ESL", "35-40 minute lesson", "Six Classendo visual cards"].map((item) => <div key={item} className="flex items-center gap-3 text-sm font-semibold"><span className="grid h-8 w-8 place-items-center rounded-full bg-[#eaf3e5] text-[#598049]"><Check size={16} /></span>{item}</div>)}
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-6 py-16">
        <div className="max-w-2xl"><p className="text-sm font-bold uppercase tracking-[0.19em] text-[#6f9560]">Ready to teach</p><h2 className="mt-3 text-3xl font-semibold">Everything you need for one focused animal lesson</h2></div>
        <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {["Teacher overview with clear objectives", "Six large printable visual flashcards", "Visual matching worksheet", "Trace-and-write worksheet", "Animal Corners speaking activity", "38-minute lesson plan and answer key"].map((item) => <div key={item} className="rounded-2xl border border-[#e1e8dc] bg-white p-5 shadow-sm"><Check className="text-[#6f9560]" size={20} /><p className="mt-3 font-semibold">{item}</p></div>)}
        </div>
      </section>

      <section id="preview" className="bg-[#edf4e9] py-16">
        <div className="mx-auto max-w-7xl px-6"><p className="text-sm font-bold uppercase tracking-[0.19em] text-[#6f9560]">Pack preview</p><h2 className="mt-3 text-3xl font-semibold">Print-friendly, visual and simple to use</h2>
          <div className="mt-8 grid gap-6 md:grid-cols-3"><img src={`${asset}/animals-vocabulary-full-pack-pinterest.png`} alt="Animals vocabulary lesson pack cover preview" className="w-full rounded-3xl border border-[#d2e0ca] bg-white shadow-md" /><img src={`${asset}/animals-vocabulary-matching-worksheet-pinterest.png`} alt="Animals matching worksheet preview" className="w-full rounded-3xl border border-[#d2e0ca] bg-white shadow-md" /><img src={`${asset}/animals-vocabulary-speaking-activity-pinterest.png`} alt="Animal Corners speaking activity preview" className="w-full rounded-3xl border border-[#d2e0ca] bg-white shadow-md" /></div>
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-6 py-16"><div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr]"><div><p className="text-sm font-bold uppercase tracking-[0.19em] text-[#6f9560]">Vocabulary</p><h2 className="mt-3 text-3xl font-semibold">Six essential animal words</h2><p className="mt-4 leading-7 text-[#5c665c]">The pack deliberately uses six cards so it matches Classendo&apos;s signed-out visitor experience and keeps the lesson manageable for beginners.</p></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{cards.map((word) => <div key={word} className="rounded-2xl border border-[#dce8d5] bg-white p-3 text-center shadow-sm"><img src={`${asset}/${word}.png`} alt={word} className="mx-auto h-28 w-full object-contain" /><p className="mt-2 font-semibold capitalize">{word}</p></div>)}</div></div></section>
